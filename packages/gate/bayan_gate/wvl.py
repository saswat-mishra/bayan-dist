...
from bayan_core.blg.brgs import mhbq
...
def dw2(conn:sqlite3.Connection)->str:from bayan_core.blg.brgs import gkou as B;A=conn.execute('SELECT COUNT(*), MIN(record_id), MAX(record_id) FROM fingerprint_flat').fetchone();return B(f"{A[0]}|{A[1]}|{A[2]}".encode())
