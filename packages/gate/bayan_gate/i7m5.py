...
def wos(ts:datetime,kind:str=_h)->str:return f"{ts.year}-Q{(ts.month-1)//3+1}"if kind==_h else f"{ts.year}-{ts.month:02d}"
class b8d:
	...
	def assert_not_suspended(B,dep:sqlite3.Row)->_A:
		A=dep
		if A[_Q]is not _A:raise u5fa(423,f"deployment {A[_B]} is suspended: {A[_i]}; a principal with authority must clear it",suspended=_R,leafIndex=A[_j])
	def pack_for(D,dep:sqlite3.Row)->k2x:
		...
		return A
	def deployment_status(C,dep_id:str)->dict[str,Any]:F='clientLogRetention';E='vendorDisposal';D=dep_id;A=C.deployment(D);B=C.packs[A[_K]];return{_B:D,'pack':{_B:B.id,_F:B.version,_E:B.digest,'accepted':A[_L],'pinned':A[_L]==B.digest or C.pack_upgrade_exists(A,B.digest),'activated':list(B.activated)},'suspended':A[_Q]is not _A,'suspension':{'at':A[_Q],'reason':A[_i],'leaf':A[_j]}if A[_Q]else _A,'acceptance':C._acceptance_summary(D),'trustDir':str(C.cfg.data_dir/'trust'),'outboxDir':str(C.cfg.outbox_dir),'retention':{E:B.retention.get(E,'P90D'),F:B.retention.get(F)},'primaryFramework':B.primary_framework,'nameAr':A['name_ar']or A[_C],_C:A[_C]}
	def _acceptance_summary(D,dep_id:str)->dict[str,Any]|_A:
		...
		if A is _A:return
		C=json.loads(A['document']);return{_E:A[_E],B:A['accepted_by'],'at':C.get(B,{}).get('at'),'components':sorted(A for A in C if A not in('schema',_a,_E,B,'signature'))}
	def pack_upgrade_exists(A,dep:sqlite3.Row,to_digest:str)->bool:from bayan_gate.xb2b import knu as B;return any(A['predicate']['to'][_E][_S]==to_digest for(A,B)in B(A,dep,_k))
	...
	def upgrade_pack(B,dep_id:str,actor:str,reason:str)->dict[str,Any]:
		...
		B.events.emit(_k,deployment=D,pack=A.id,version=A.version,digest=A.digest,leaf=J,by=F);return{_a:D,'pack':A.id,_F:A.version,_E:A.digest,'leafIndex':J}
	...
	def source_classes(D,dep_id:str)->dict[str,c5aj]:
		E=dep_id;from bayan_core.evxn import xd0 as F;G=D.deployment(E);B={A:c5aj(B[_T])for(A,B)in D.packs[G[_K]].raw.get(_l,{}).items()}
		...
		return B
	def field_tags(A,dep_id:str)->dict[str,frozenset[str]]:B=A.deployment(dep_id);return{B:frozenset(A.get(_b,[]))for(B,A)in A.packs[B[_K]].raw.get(_l,{}).items()if A.get(_b)}
	...
	def recipient_extra(C,dep:sqlite3.Row,requester:str)->dict[str,Any]:
		...
		if A is _A:return{}
		return{B:A[B],_m:{_S:A[_n]},'acknowledgement':{_S:A['ack_digest']},'validUntil':datetime.fromtimestamp(A['valid_until'],timezone.utc).strftime(_Z)}
	...
	def skills(C,dep_id:str,answers:str|_A=_A,awaiting:bool=_c)->list[dict[str,Any]]:
		N='example';M='description_ar';L='description';G=answers;F='answers';D=dep_id;from bayan_core.evxn import l0zv as O;P,Q=C.ratified(D),C.field_classes(D);R,H=C.source_classes(D),C.field_tags(D);I=[]
		for A in C.db.execute('SELECT * FROM skill WHERE deployment_id=? ORDER BY name, version',(D,)):
			B=json.loads(A['spec'])
			if G and G not in B.get(F,[]):continue
			if awaiting and A[_H]is not _A:continue
			J=[A if isinstance(A,dict)else{_C:A}for A in B.get(_I,[])];K=sui.from_dict(B);E=O(K,P,classes=Q,source_classes=R,tags=H);S=E.cap_reasons;T=[{_C:A.name,_T:A.field_class.value,_d:A.transform.value if A.transform else _A,'skillTags':sorted(B.tags),'packTags':sorted(H.get(A.name,())),_b:sorted(A.tags),'sources':list(E.lineage.get(A.name,()))}for(A,B)in zip(E.manifest.fields,K.output_schema.columns)];I.append({_C:A[_C],_F:A[_F],'riskClass':A['risk_class'],'maxGradeD':A[_U],'capReasons':[A.to_json()for A in S],'columns':T,F:B.get(F,[]),L:B.get(L,''),M:B.get(M,''),_o:json.loads(A[_p]),_I:[A[_C]for A in J],'paramExamples':{A[_C]:A.get(N)for A in J if A.get(N)is not _A},'certified':A[_H]is not _A,'certifiedBy':A['certified_by'],'certifiedAt':time.strftime(_Z,time.gmtime(A[_H]))if A[_H]else _A,_M:bool(A[_M]),_e:A[_e],'bundleDigest':A['bundle_digest']})
		return I
	...
	def _summaries(B,dep:sqlite3.Row)->list[zdp0]:
		...
		for A in B.db.execute('SELECT * FROM skill WHERE deployment_id=? AND decertified=0',(C[_B],)):E=sui.from_dict(json.loads(A['spec']));F=B.skill_manifest(C[_B],E);H=3 if A[_H]is not _A else 2;I=apgg(A[_U],F,f30(F),G,H);D.append(zdp0(A[_C],A[_F],frozenset(E.answers),A[_U],I,A[_U]<=2))
		return D
	...
	def run(A,dep_id:str,skill:str,version:str,params:dict[str,Any],requester:str,dryrun:bool=_c)->dict[str,Any]:
		...
		A.events.emit('run',run=J,skill=f"{C}@{D}",status=_q if G else _f,certificate=T.label,certificate_us=U,dryrun=H);return A.get_run(J)
	def insert_run(C,run_id:str,dep_id:str,skill:str|_A,version:str|_A,requester:str,params:str,rows:list[dict[str,Any]],output_digest:str,input_digest:str,manifest:fil6,prov:si1,cert:bre,us:int,quarantine:dict[str,Any]|_A,dryrun:int|bool,*,output_schema:t2ji,derived_from:str|_A=_A,transform_digest:str|_A=_A,lookup:dict[str,Any]|_A=_A)->_A:B=lookup;A=quarantine;C.db.execute('INSERT INTO run (id, deployment_id, skill_name, skill_version, requester, params, status, rows, output_digest, input_digest, manifest, certificate, certificate_us, quarantine, dryrun, provenance, derived_from, transform_digest, output_schema, created_at, lookup) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',(run_id,dep_id,skill,version,requester,params,_q if A else _f,json.dumps(rows),output_digest,input_digest,json.dumps(q5e8(manifest)),json.dumps(qxd5(cert)),us,json.dumps(A)if A else _A,int(dryrun),json.dumps(w8iy(prov)),derived_from,transform_digest,json.dumps(output_schema.to_dict()),int(time.time()),json.dumps(B)if B else _A))
	...
	def run_row(C,run_id:str)->sqlite3.Row:
		...
		return B
	def _menu(B,row:sqlite3.Row,target:int)->n4y:A=row;C=B.deployment(A[_g]);D=vf4(json.loads(A[_V]));E=3 if gowu(json.loads(A[_W])).certified else 2;return dsan(D,target,gra9(B.pack_for(C)),B.recipient_facts(C,A[_J]),E)
	@staticmethod
	def _recommendation(menu:n4y)->dict[str,Any]|_A:
		A=menu.recommended
		if A is _A:return
		return{_r:A.describe(),_s:[{_G:A.field,_d:A.transform.value,_I:dict(A.params)}for A in A.changes],'d':f"D{A.d}",_t:f"R{A.required_r}",_u:A.reaches_target}
	def get_run(C,run_id:str)->dict[str,Any]:
		...
		I=C._recommendation(C._menu(A,2))if B and A[_Y]==_f and int(B['d'])<2 else _A;return{_B:A[_B],_a:A[_g],'skill':A['skill_name'],_F:A['skill_version'],_Y:A[_Y],_J:A[_J],_I:json.loads(A[_I]),'outputRef':{_X:len(F),_E:A['output_digest'],'inputDigest':A['input_digest']},_X:F[:500],D:B,'certificateMicros':A['certificate_us'],_N:json.loads(A[_N])if A[_N]else _A,G:bool(A[G]),_V:json.loads(A[_V]),_W:json.loads(A[_W]),'derivedFrom':A['derived_from'],'transformDigest':A['transform_digest'],_o:json.loads(A[_p]),E:json.loads(A[E])if A[E]else _A,_v:I}
	def uplift_menu(B,run_id:str,target:int)->dict[str,Any]:C='recommended';D=B.run_row(run_id);A=B._menu(D,target);return{'target':f"D{A.target_d}",'current':f"D{A.current_d}",'asyncRequired':A.async_required,_v:B._recommendation(A),'unreachableReason':A.unreachable_reason,'options':[{_s:[{_G:A.field,_d:A.transform.value,_I:dict(A.params)}for A in A.changes],_r:A.describe(),_u:A.reaches_target,'d':f"D{A.d}",_t:f"R{A.required_r}",'cost':A.cost,'loses':list(A.loses),'keeps':A.keeps,C:A.recommended}for A in A.options],C:A.recommended.describe()if A.recommended else _A}
	...
	def upgrade_d3(A,run_id:str)->dict[str,Any]:
		...
		def E()->_A:
			...
			with A.tx:L=A.deployment(B[_g]);M=json.loads(B[_X]or'[]');E=vf4(json.loads(B[_V]));F=[K(M,A.name,int(A.param(I)),oy60())for A in E.fields if A.param(I)is not _A];G=J(E,F);N=gowu(json.loads(B[_W]));H,O=A._certify(L,G,N,B[_J]);A.db.execute('UPDATE run SET certificate=?, manifest=? WHERE id=?',(json.dumps(qxd5(H)),json.dumps(q5e8(G)),D));A.jobs[C].update(status='done',certificate=H.label,properties=[{_C:A.name,'threshold':A.threshold,'observed':A.observed,'passed':A.passed}for A in F])
