...
i7nm=_C,_E,_B,_F,_G,_D,_H
jrtv:dict[str,tuple[str,str]]={_C:('Releases now','يُفرج عنه الآن'),_E:('Needs review','يحتاج إلى مراجعة'),_B:('Blocked, fixable by transformation','محجوب، يمكن إصلاحه بالتحويل'),_F:('Blocked, change the recipient','محجوب، غيّر المستلم'),_G:('Blocked, roster action needed','محجوب، يلزم إجراء في سجل التكليف'),_D:('Blocked by the budget or a suspension','محجوب بالميزانية أو بالإيقاف'),_H:('Quarantined','محجور')}
wsfx=frozenset({'drop_field','truncate'})
hb9=frozenset({'change_recipient','legal_instrument'})
q58=frozenset({'roster','not_evaluable'})
@dataclass(frozen=True)
class etf:quarantined:bool=_I;budget_exhausted:bool=_I;suspended:bool=_I;reviewers_needed:int|_A=_A
@dataclass(frozen=True)
class fi7:
	kind:str;en:str;ar:str
	def to_json(A)->dict[str,str]:return{'kind':A.kind,'en':A.en,'ar':A.ar}
nypa=fi7(_H,"The output did not conform to the skill's declared schema. Nothing can be released from this run.",'المخرجات لم تطابق المخطط المعلن للمهارة. لا يمكن الإفراج عن أي شيء من هذا التشغيل.')
pjm=fi7(_D,'Cannot release — the deployment is suspended after a sensor event. An officer with authority must clear it.','لا يمكن الإفراج — النشر موقوف بعد حدث من المستشعر. يجب أن يرفع الإيقاف مسؤول ذو صلاحية.')
cq2=fi7(_D,'Cannot release — the period budget for this cohort is exhausted. Wait for the next period or ask the lead.','لا يمكن الإفراج — ميزانية الفترة لهذه المجموعة مستنفدة. انتظر الفترة التالية أو اسأل قائد التسليم.')
...
def cbp(c:bre,facts:etf=etf(),*,threshold:int=2)->fi7:
	D=facts
	if D.quarantined:return nypa
	if D.suspended:return pjm
	if D.budget_exhausted:return cq2
	A=das(c)
	if A is not _A:
		if A.remedy_kind in q58:return fi7(_G,f"Cannot release — {A.name}: {A.detail}. Renew the roster entry; no transformation fixes this.",f"لا يمكن الإفراج — {A.name}: {A.detail}. جدّد قيد السجل؛ لا يصلح هذا أي تحويل.")
		if A.remedy_kind in hb9:return fi7(_F,f"Cannot release to this recipient — {A.name}: {A.detail}. Change the recipient; no transformation fixes this.",f"لا يمكن الإفراج لهذا المستلم — {A.name}: {A.detail}. غيّر المستلم؛ لا يصلح هذا أي تحويل.")
	...
	if c.releasable and c.required_r<=1:F=', '.join(A.target for A in c.findings if A.action=='modify')or'declared counts only';return fi7(_C,f"No reviewer needed. D{c.d}: {F} transformed as declared.",f"لا حاجة إلى مراجع. D{c.d}: تم تحويل {F} كما هو معلن.")
	...
	G='s'if C!=1 else'';return fi7(_E,f"Needs {C} blinded reviewer{G} — {k0y(c)}.",f"يحتاج إلى {C} مراجع(ين) محجوبين — {fd92(c)}.")
