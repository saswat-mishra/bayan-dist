...
def htz(schema:t2ji,ratified:frozenset[str]|_A=_A,sensitive_declared:frozenset[str]=frozenset(),mechanism:str='output-check',row_level:bool=_H,classes:Mapping[str,c5aj]|_A=_A,tags:Mapping[str,frozenset[str]]|_A=_A)->fil6:B=ratified;A=tuple(im5(A.name,(classes or{}).get(A.name,A.field_class),A.transform,A.params,ratified=B is _A or A.name in B,tags=A.tags|frozenset((tags or{}).get(A.name,())),load_bearing=A.load_bearing)for A in schema.columns);return fil6(A,sensitive_declared,row_level,mechanism)
