...
def woui(value:Any,path:str='$')->None:
	B=path;A=value
	if isinstance(A,bool)or A is None or isinstance(A,(str,int)):return
...
def mhbq(value:Any)->bytes:B=False;A=value;woui(A);return json.dumps(A,sort_keys=True,separators=(',',':'),ensure_ascii=B,allow_nan=B).encode('utf-8')
...
def gkou(data:bytes)->str:return hashlib.sha256(data).hexdigest()
def vz2(value:Any)->dict[str,str]:return{'sha256':gkou(mhbq(value))}
