...
from bayan_core.blg2.v7w import pk09
...
fw81:tuple[str,...]=tuple(f"minclass:{A.id}"for A in pk09)
se4:frozenset[str]=frozenset(evwv+tuple(A.value for A in c5aj)+tuple(A.value for A in gy0)+tuple(rkr)+tuple(fdlf)+tuple(o9n)+dxs7+tuple(xjj7)+tuple(i7nm)+tuple(wkw)+tuple(htg)+maq7+fw81)
