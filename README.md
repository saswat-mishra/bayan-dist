...
**A signed, recomputable Compliance Certificate for every diagnostic data release from an air-gapped AI deployment. Distribution build v0.3.0.**
...
Then open the console and switch **Acting as** between the six roles. Every role home opens with one line — *what you can do here* — and every code on every screen (a grade, a field class, a transform, a gate, a mechanism) is rendered in the client's own words from the policy pack, in English and Arabic; the technical detail (digests, paths, the certificate's tracks) sits in a collapsed *Technical* tier.

| Role | Home answers |
|---|---|
| **Omar** — engineer (vendor FDE) | What can I get out, at what price, right now? Six steps that live in the URL: question priced in words → skill with the true cap reason → run → improve → request → **Handoff** (retention as a date, the bundle as one download, the register line, the verify command with the real trust directory, a pseudonym lookup through the ordinary review path). |
| **Layla, Faisal** — reviewers (client ISO; Layla holds authority) | What am I being asked to sign, and what would I be responsible for? An onboarding checklist until this browser holds an approved key; an inbox grouped by what each request needs from you; a brief in three tiers — decision, explanation, technical — with nothing pre-selected and a confirmation that says what you take on; the gate finalises when the last vote lands. |
...
| **Noura** — data owner / DBA | Which fields wait for my answer, and what depends on it? Ratify or reclassify with a reason; co-sign the skills that wait for the client. |
