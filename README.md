...
**A signed, recomputable Compliance Certificate for every diagnostic data release from an air-gapped AI deployment. Distribution build v0.2.0.**
...
Then open the console and switch **Acting as** between the seven surfaces:

| Role | Home answers |
|---|---|
| **Omar** — engineer (vendor FDE) | What can I get out, at what price, right now? A six-step wizard: question → skill → run → improve → request → track. |
| **Layla, Faisal** — reviewers (client ISO; Layla holds authority) | What am I being asked to sign, and what would I be responsible for? A brief in four blocks, no grade pill, no Resolve button — the gate finalises when the last vote lands. |
| **Priya** — delivery lead (vendor) | Is acceptance evidence flowing, and where is it stuck? |
| **Khalid** — auditor (client compliance) | Show me the evidence for control X over period Y — framework → control → evidence → receipt in three actions. |
| **Hessa** — external assessor | The auditor's view, read-only, artefacts as digests only, with "how to verify offline" first. |
| **Noura** — data owner / DBA | Which vendor-proposed field classes await my ratification? |
