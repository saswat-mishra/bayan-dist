...
from data.mock.identifiers import hrci,hka4,azg
k43q=[_B,'leave','payroll','it-access','housing','medical-insurance','training']
xfn7=[_K,_L,_M,_N,_G,_O]
hwmj=[_A,_C,_E,_H,_F,_I]
...
def lxaf(rng:random.Random,ts_ms:int)->str:from bayan_core.zm0.sue import nzm as A;return A(ts_ms,rng.getrandbits(80))
...
def nuk0(ms:float)->str:
	for(A,B)in zip(jzr2,(100,500,1000,5000,30000)):
		if ms<B:return A
	return'>30s'
def umfr(n:int=50000,days:int=90,seed:int=42,deployment_id:str='moi-itsm-prod-01',end:datetime|_D=_D,content_share:float=.03)->list[tuple[dict[str,Any],dict[str,Any]|_D]]:
	...
	for h in range(n):
		...
		G=A.choices(k43q,weights=[30,20,15,12,8,8,7])[0];i=B>=U and G==_B;V=41 if B<U else 42;K=.36 if i else .035;C=_A;L=A.random()
		if L<K:C=_C
		elif L<K+.02:C=A.choice([_E,_H,_I])
		elif L<K+.03:C=_F
		E=0 if C==_C else A.randint(2,8);j=[_K,_L,f"topic-{G}",_M]+([_N,_G]if E else[_G])+[_O];k='error'if C in(_E,_I)else'content_filter'if C==_F else'stop';W,X=A.randint(40,900),A.randint(20,400);Y=A.uniform(300,4800)+(2500 if C==_E else 0);l=int(B.timestamp()*1000);m=A.sample(hrci[:12]if G==_B else hrci,k=min(E,4))if E else[];M=C==_F or A.random()<.012;F:dict[str,Any]={'schemaVersion':'bayan.fingerprint/v1','recordId':lxaf(A,l),'deploymentId':S,Q:{'scheme':'ae.ia-reg.v1_1','tier':'Sensitive'if M else'Confidential',R:3 if M else 2,'basis':'inherited-from-system'},'productVersion':'4.2.1','promptVersion':{I:f},'modelVersion':{I:g},'indexVersion':{'generation':V,b:{I:hashlib.sha256(f"index-{V}".encode()).hexdigest()}},'operation':'chat','route':j,'finishReason':k,'errorCode':C,'latency':{'bucket':nuk0(Y),'ms':round(Y,1)},'tokens':{'inputBucket':mrta(W),'outputBucket':mrta(X),'inputExact':W,'outputExact':X},a:{c:B.strftime('%Y-%m-%dT%H:00:00Z'),'exact':B.strftime('%Y-%m-%dT%H:%M:%S.000Z')},'conversationId':l3rg(T,f"conv-{h//3}"),'retrieval':{'hitCount':E,'docRefs':[{'ref':l3rg(T,B),'kFloorMet':True,'rank':A+1}for(A,B)in enumerate(m)]},'toolCalls':{'count':1 if G==_B and E else 0,**({'names':['pension_lookup']}if G==_B and E else{})}}
		...
		N=_D
		if A.random()<content_share:D=azg(A);O=A.randrange(len(uqz));Z=A.random()<.55;o=ifl[O]if Z else uqz[O];P=o.format(eid=D.emirates_id,name=D.name_ar if Z else D.name_en,iban=D.iban,mobile=D.mobile,hijri=hka4(B.date()).split(' ')[0]);N={d:hashlib.sha256(P.encode()).hexdigest(),'prompt_text':P,'response_text':nt5m[O]if C==_A else nt5m[5],'extra':{'name_ar':D.name_ar,'name_en':D.name_en,'department':D.department,'address':D.address,'makani':D.makani,'hijri':hka4(B.date())}};F['contentRef']={b:{I:N[d]},'storeId':'enclave-content-01','bytes':len(P.encode())};F[Q][R]=max(F[Q][R],2)
		J.append((F,N))
	J.sort(key=lambda rc:rc[0][a][c]);return J
def op2c(n:int=3000,seed:int=7)->list[dict[str,Any]]:
...
def pg9(n:int=1200,seed:int=11)->list[dict[str,Any]]:
	...
	return C
def tdl(n:int=2500,seed:int=5)->list[dict[str,Any]]:
