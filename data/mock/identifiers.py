...
def nxq(rng:random.Random,birth_year:int|None=None)->str:A=birth_year or rng.randint(1960,2004);B=f"784{A}{x35w(rng,7)}";return f"784-{A}-{B[7:14]}-{pbgf(B)}"
def sfo(rng:random.Random)->str:return'1'+x35w(rng,9)
...
def azg(rng:random.Random)->dc63:A=rng;B,C=A.choice(sadm);return dc63(B,C,nxq(A),gim(A),m1lr(A),en3(A),A.choice(l3gc),A.choice(bz7))
